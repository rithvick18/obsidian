...
const defaultIncident: any = {
  id: 'N/A',
  title: 'No Incident Selected',
  timeAgo: '0m',
  severity: 'medium',
  tags: ['System Audit'],
  impactedEntity: 'Infrastructure Edge',
  assignee: 'SYSTEM_ENGINE',
...
export default function IncidentsView() {
  ...
  const activeIncidentList: any[] = incidents && incidents.length > 0 ? incidents : [defaultIncident];
  const selectedIncident = (activeIncidentList.find(inc => inc.id === selectedIncidentId) || activeIncidentList[0]) as any;
  ...
  const currentRiskScore = selectedIncident.risk_score !== undefined ? selectedIncident.risk_score : (selectedIncident.severity === 'critical' ? 100 : 75);
  const targetUserNode = selectedIncident.user_id || selectedIncident.impactedEntity || 'unknown_node';
  const displayActionContext = selectedIncident.action || selectedIncident.title;
